...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh root@${node_ip} "mkdir -p /opt/k8s/kube-nginx/sbin"
...
stream {
    upstream backend {
        ...
        server 192.168.56.107:6443         max_fails=3 fail_timeout=30s;
        server 192.168.56.108:6443         max_fails=3 fail_timeout=30s;
    }
